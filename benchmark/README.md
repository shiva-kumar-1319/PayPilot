# PayPilot 4-Way Comparative Benchmark Framework

The **PayPilot Benchmark Framework** provides an objective, adversarial, and reproducible evaluation environment for automated payment recovery systems. It evaluates 4 competing strategies on identical, grounded failure event sequences without synthetic bias or fabricated numbers.

---

## 1. Ground Truth Separation Architecture

A primary flaw in naive evaluators is "ground truth leakage," where the agent has direct access to the hidden reasons behind a payment failure. PayPilot strictly enforces two isolated layers:
...
| **4. PayPilot** | Calibrated ML + Policy Gate | Bounded ReAct | Cost-Aware Net Expected Value (EV) Maximization |
...
3. **Hard-Stop Violation Count**: Number of actions attempted on hotlisted/fraudulent transactions. **PayPilot guarantees exactly 0**.
...
PayPilot (Cost-Aware EV Agent) |   59.30% | INR 3,779,082.18 | INR 3,757,329.78 |          0
----------------------------------------------------------------------------------------

[SUMMARY LIFT]
* PayPilot Net Revenue vs Blind Retry:     +INR 3,188,277.25
* PayPilot Net Revenue vs Rule Heuristic:  +INR 40,783.36
* PayPilot Hard-Stop Violations:           0 (Invariant: ZERO)
