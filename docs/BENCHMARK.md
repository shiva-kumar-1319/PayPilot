# PayPilot 4-Way Comparative Benchmark Specification (`BENCHMARK.md`)
...
| **PayPilot Agent** | Cost-Aware EV Optimization | 593 / 1,000 | 59.30% | INR 3,789,975.60 | INR 32,645.82 | **INR 3,757,329.78** | **0** |
...
1. **Massive Net Revenue Lift over Blind Retries**:
   - PayPilot delivers **+INR 3,188,277.25 net revenue lift** (+560.3% financial lift) over naive blind retries.
...
2. **Strict Zero Hard-Stop Violations**:
   - While Blind Immediate Retry committed **62 severe policy violations** by re-hammering hard-stop codes (`FRAUD_REJECTED`, `STOLEN_CARD`, `INVALID_ACCOUNT`), PayPilot achieved strictly **0 violations**.

3. **Intelligent Rail Selection**:
   - PayPilot does not blindly retry cards; it recognizes bank issuer declines (ISO 05 Do Not Honor) and autonomously pivots to 1-click UPI intents or scheduled exponential backoff retries.
